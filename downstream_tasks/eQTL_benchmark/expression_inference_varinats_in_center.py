...
class DualRegionInferenceDataset(Dataset):
    ...
    def wrap_seq(self, ids_tss_pre_variant, ids_tss_post_variant, ids_variant):
        # --- Wrap with CLS / SEP ---
        cls_id = self.gen_tokenizer.cls_token_id
        sep_id = self.gen_tokenizer.sep_token_id
        gap_id = self.gen_tokenizer.encode_plus("N"*1000)
        assert cls_id is not None and sep_id is not None, "Tokenizer must have CLS/SEP"

        tok_tss_pre_variant = torch.tensor(ids_tss_pre_variant, dtype=torch.long)
        tok_tss_post_variant = torch.tensor(ids_tss_post_variant, dtype=torch.long)
        tok_variant = torch.tensor(ids_variant, dtype=torch.long)
        try:
            seq_input_ids  = torch.cat([tok_tss_pre_variant.new_tensor([cls_id]), 
                                        tok_tss_pre_variant,
                                        tok_variant,
                                        tok_tss_post_variant,
                                        tok_tss_pre_variant.new_tensor([sep_id])])
        except:
            print(tok_tss_post_variant.new_tensor([cls_id]).shape, torch.tensor(5, dtype=torch.long).shape, tok_tss_pre_variant.shape ,tok_variant.shape,  tok_tss_post_variant.shape, tok_tss_post_variant.new_tensor([sep_id]).shape)
            exit(1)
        ...
        if seq_input_ids.shape[0] < MAX_TOKENS:
            print(f"Token length {seq_input_ids.shape[0]} smaller then {MAX_TOKENS}")
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        try:
            ref_seq, alt_seq = self._fetch_seq_ref_alt(chrom_raw, variant_pos, VARIANT_REGION_BP, ref_base, alt_base)
        ...
        if tss_info is None:
            ...
            tokens_df = self.tokenize_genome(self.genome_genes, chrom_raw, tss_pos, tss_info['reverse'])
        ids_tss = tokens_df["token_id"].values.astype(np.int32)
        L = min(ids_tss.shape[0], MAX_CONTENT_TOKENS)
        ids_tss = ids_tss[:L]
        ids_tss_pre_variant = ids_tss[:len(ids_tss)//2-VARIANT_REGION_TOKENS-50]
        ids_tss_post_variant = ids_tss[len(ids_tss)//2-50:]
        if return_type != 'tss_only':
            ids_tss = ids_tss[VARIANT_REGION_TOKENS:]
            seq_input_ids, seq_attn_mask, seq_token_type = self.wrap_seq(ids_tss_pre_variant, ids_tss_post_variant, ids_variant)
        else:
            cls_id = self.gen_tokenizer.cls_token_id
            sep_id = self.gen_tokenizer.sep_token_id
            assert cls_id is not None and sep_id is not None, "Tokenizer must have CLS/SEP"
            tok_tss = torch.tensor(ids_tss, dtype=torch.long)
            seq_input_ids  = torch.cat([tok_tss.new_tensor([cls_id]), tok_tss, tok_tss.new_tensor([sep_id])])
            assert seq_input_ids.shape[0] <= MAX_TOKENS, (
                f"Token length {seq_input_ids.shape[0]} exceeds {MAX_TOKENS}"
            )
            seq_attn_mask  = torch.ones(seq_input_ids.size(0), dtype=torch.long)
...
class CollateFn:
    ...
    def collate_fn(cls, batch):
        ...
        special_keys = ['gene_id', 'signal_id', 'type', 'variant_pos', 'ref_base', 'alt_base']
...
def run_inference(model, dataloader, device) -> dict[tuple[str, str], dict[str, float]]:
    ...
    with torch.autocast(device_type="cuda", dtype=torch.bfloat16), torch.no_grad():
        for batch in dataloader:
            ...
            for signal_id, gene_id, variant_pos, ref_base, alt_base, val, type in zip(
                batch["signal_id"],
                batch["gene_id"],
                batch["variant_pos"], 
                batch['ref_base'],
                batch['alt_base'], logits, batch["type"]
            ):
                #print('Ended', gene_id, signal_id)
                column_name = f"predicted_expression_{type}"
                if (signal_id, gene_id, variant_pos, ref_base, alt_base) in results:
                    results[(signal_id, gene_id, variant_pos, ref_base, alt_base)][column_name] = val
                else:
                    results[(signal_id, gene_id, variant_pos, ref_base, alt_base)] = {
                        column_name: val
                    }
    return results
...
def main() -> None:
    ...
    if global_rank == 0:
        rows = [
            {"common_variant_analysis_signal_id": sig, "gene_id": gene, 'variant_pos':variant_pos,'ref_base':ref_base, 'alt_base':alt_base , **vals}
